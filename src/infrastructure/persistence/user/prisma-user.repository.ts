...
import { User, ProfileStatus, UserProps } from '../../../core/domain/user/entities/user.entity';
...
class ProfileStatusMapper {
    private static readonly toPrismaMap: Record<ProfileStatus, PrismaProfileStatus> = {
    ...
    private static readonly toDomainMap: Record<PrismaProfileStatus, ProfileStatus> = {
...
export class PrismaUserRepository implements IUserRepository {
    ...
    private mapToDomain(data: any): User {
        const props: UserProps = {
            ...
            profession: data.profession ? Profession.create(data.profession) : undefined,
            photo: data.photo ? UserPhoto.create(data.photo) : undefined,
            ageGroup: data.ageGroup ? UserAgeGroup.create(data.ageGroup) : undefined,
            nationality: data.nationality ? Nationality.create(data.nationality) : undefined,
    ...
    async updateProfileStatus(userId: string, status: ProfileStatus): Promise<User> {
        const updated = await this.prisma.user.update({
            where: { id: userId },
            data: {
    ...
    async search(
        ...
        const fields = options?.fields || ['email', 'username', 'fullName', 'nickname'];
