        ...
        return Document.create(
            {
                ...
                storageProvider: (raw.storageProvider as StorageProvider) || StorageProvider.CLOUDINARY,
                ...
                verificationStatus: (raw.verificationStatus as DocumentVerificationStatus) || DocumentVerificationStatus.PENDING,
...
export class PrismaDocumentRepository implements IDocumentRepository {
    ...
    async list(options?: ListDocumentsOptions): Promise<Document[]> {
        const records = await this.prisma.document.findMany({
            where: {
                ...
                ...(options?.legalOpinionId && { legalOpinionId: options.legalOpinionId }),
                ...(options?.serviceRequestId && { serviceRequestId: options.serviceRequestId }),
                ...(options?.litigationCaseId && { litigationCaseId: options.litigationCaseId }),
                ...(options?.supportTicketId && { supportTicketId: options.supportTicketId }),
                ...(options?.searchTerm && {
                    OR: [
    ...
    async count(options?: Omit<ListDocumentsOptions, 'limit' | 'offset'>): Promise<number> {
    ...
    async findByRequest(options: DocumentsByRequestOptions): Promise<Document[]> {
        const whereClause = this.getRequestWhereClause(options.requestType, options.requestId);
    ...
    async verifyMultiple(ids: string[], verifiedBy: string, notes?: string): Promise<number> {
    ...
    async deleteByRequest(requestType: DocumentRequestType, requestId: string): Promise<number> {
    ...
    async getStatistics(options?: {
        ...
        if (options?.requestType && options?.requestId) {
            Object.assign(whereClause, this.getRequestWhereClause(options.requestType, options.requestId));
        }
