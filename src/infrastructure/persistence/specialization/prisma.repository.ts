  ...
  implements ISpecializationRepository
{
  ...
  async count(
    options?: Omit<ListSpecializationsOptions, 'limit' | 'offset'>,
  ): Promise<number> {
...
export class PrismaProviderSpecializationRepository
  implements IProviderSpecializationRepository
{
  constructor(private readonly prisma: PrismaClient) {}
  ...
  async create(
    ...
    const data = ProviderSpecializationMapper.toPersistence(
      providerSpecialization,
    );
  ...
  async list(
    options?: ListProviderSpecializationsOptions,
  ): Promise<ProviderSpecialization[]> {
    const records = await this.prisma.providerSpecialization.findMany({
      ...
      take: options?.limit,
      skip: options?.offset,
      orderBy: [{ caseCount: 'desc' }, { successRate: 'desc' }],
    });
    return records.map(ProviderSpecializationMapper.toDomain);
  }

  async count(
    options?: Omit<ListProviderSpecializationsOptions, 'limit' | 'offset'>,
  ): Promise<number> {
  ...
  async findBySpecialization(
    options: ProvidersBySpecializationOptions,
  ): Promise<ProviderSpecialization[]> {
    const records = await this.prisma.providerSpecialization.findMany({
      where: {
        specializationId: options.specializationId,
      ...
      take: options.limit,
      skip: options.offset,
      orderBy: [{ successRate: 'desc' }, { caseCount: 'desc' }],
    });
    return records.map(ProviderSpecializationMapper.toDomain);
  }
  ...
  async existsByProviderAndSpecialization(
    providerId: string,
    specializationId: string,
  ): Promise<boolean> {
