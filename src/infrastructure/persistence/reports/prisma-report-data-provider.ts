...
export class PrismaReportDataProvider implements IReportDataProvider {
  ...
  async getFinancialStats(dateRange: DateRange): Promise<FinancialStats> {
    const [activeMemberships, transactions, payments, pendingPayments] =
      await Promise.all([
        // Active memberships
  ...
  async getOperationalStats(dateRange: DateRange): Promise<OperationalStats> {
    const [requestCounts, activeProviders, activeSubscribers, slaStats] =
      await Promise.all([
  ...
  async getRequestCountsByType(dateRange: DateRange): Promise<RequestCounts> {
    const [consultations, legalOpinions, litigations, calls, services] =
      await Promise.all([
  ...
  async getProviderWorkloadStats(): Promise<{
    ...
    const nameMap = new Map(
      providerNames.map((p) => [p.id, p.fullName || p.username || 'Unknown']),
    );
    ...
    const totalCompleted = topPerformers.reduce(
      (sum, p) => sum + p.completedRequests,
      0,
    );
    const averageWorkload =
      activeProviders > 0 ? totalCompleted / activeProviders : 0;
