    ...
    async findAll(
        ...
        const [data, total] = await Promise.all([
            this.prisma.report.findMany({
                ...
                skip: offset,
                orderBy: {
                    [pagination?.orderBy || 'generatedAt']: pagination?.orderDirection || 'desc',
                },
            }),
...
@Injectable()
export class PrismaAnalyticsMetricRepository implements IAnalyticsMetricRepository {
    constructor(private readonly prisma: PrismaService) {}
    ...
    async findById(id: string): Promise<AnalyticsMetric | null> {
        const metric = await this.prisma.analyticsMetric.findUnique({ where: { id } });
    ...
    async findAll(
        ...
        const [data, total] = await Promise.all([
            this.prisma.analyticsMetric.findMany({
                ...
                skip: offset,
                orderBy: {
                    [pagination?.orderBy || 'timestamp']: pagination?.orderDirection || 'desc',
                },
            }),
    ...
    async findByName(metricName: string, timeRange?: TimeRange): Promise<AnalyticsMetric[]> {
