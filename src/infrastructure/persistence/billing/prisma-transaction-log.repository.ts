...
@Injectable()
export class PrismaTransactionLogRepository implements ITransactionLogRepository {
    ...
    async markAsPartiallyRefunded(id: string): Promise<TransactionLog> {
        const updated = await this.prisma.transactionLog.update({
            where: { id },
            data: {
                status: PaymentStatusMapper.toPrisma(PaymentStatusEnum.PARTIALLY_REFUNDED),
            },
        });
        return this.toDomain(updated);
    }
    ...
    // PRIVATE HELPERS
    // ============================================
    private buildWhereClause(options?: TransactionLogListOptions | TransactionLogCountOptions) {
        const where: any = {};
