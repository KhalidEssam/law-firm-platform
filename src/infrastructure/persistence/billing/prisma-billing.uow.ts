...
import { Refund, RefundReviewData } from '../../../core/domain/billing/entities/refund.entity';
...
export class PrismaBillingUnitOfWork implements IBillingUnitOfWork {
     ...
     */
    private createTransactionalUow(tx: PrismaTransactionClient): IBillingUnitOfWork {
        return {
            ...
            transaction: async <R>(work: (uow: IBillingUnitOfWork) => Promise<R>): Promise<R> => {
...
class TransactionalRefundRepository implements IRefundRepository {
    ...
    async count(options?: RefundCountOptions): Promise<number> {
        return await this.prisma.refund.count({ where: this.buildWhereClause(options) });
    }
    ...
    async getStatistics(fromDate?: Date, toDate?: Date): Promise<RefundStatistics> {
        ...
        const [pending, approved, rejected, processed, totalAmount] = await Promise.all([
            this.prisma.refund.count({
                where: { status: RefundStatusMapper.toPrisma(RefundStatusEnum.PENDING), ...dateFilter },
            }),
            this.prisma.refund.count({
                where: { status: RefundStatusMapper.toPrisma(RefundStatusEnum.APPROVED), ...dateFilter },
            }),
            this.prisma.refund.count({
                where: { status: RefundStatusMapper.toPrisma(RefundStatusEnum.REJECTED), ...dateFilter },
            }),
            this.prisma.refund.count({
                where: { status: RefundStatusMapper.toPrisma(RefundStatusEnum.PROCESSED), ...dateFilter },
            }),
    ...
    private buildWhereClause(options?: RefundListOptions | RefundCountOptions): any {
        const where: any = {};
        if (options?.userId) where.userId = options.userId;
        if (options?.status) where.status = RefundStatusMapper.toPrisma(options.status);
...
class TransactionalDisputeRepository implements IDisputeRepository {
    ...
    async count(options?: DisputeCountOptions): Promise<number> {
        return await this.prisma.dispute.count({ where: this.buildWhereClause(options) });
    }
    ...
    async findEscalatedDisputes(): Promise<Dispute[]> {
        const found = await this.prisma.dispute.findMany({
            where: { status: DisputeStatusMapper.toPrisma(DisputeStatusEnum.ESCALATED) },
    ...
    async getStatistics(fromDate?: Date, toDate?: Date): Promise<DisputeStatistics> {
        ...
        const [open, underReview, escalated, resolved, closed, byPriority] = await Promise.all([
            this.prisma.dispute.count({ where: { status: DisputeStatusMapper.toPrisma(DisputeStatusEnum.OPEN), ...dateFilter } }),
            this.prisma.dispute.count({ where: { status: DisputeStatusMapper.toPrisma(DisputeStatusEnum.UNDER_REVIEW), ...dateFilter } }),
            this.prisma.dispute.count({ where: { status: DisputeStatusMapper.toPrisma(DisputeStatusEnum.ESCALATED), ...dateFilter } }),
            this.prisma.dispute.count({ where: { status: DisputeStatusMapper.toPrisma(DisputeStatusEnum.RESOLVED), ...dateFilter } }),
            this.prisma.dispute.count({ where: { status: DisputeStatusMapper.toPrisma(DisputeStatusEnum.CLOSED), ...dateFilter } }),
            Promise.all([
        ...
        return {
            ...
            byPriority: { low: byPriority[0], normal: byPriority[1], high: byPriority[2], urgent: byPriority[3] },
        };
    }
    ...
    private buildWhereClause(options?: DisputeListOptions | DisputeCountOptions): any {
        const where: any = {};
        if (options?.userId) where.userId = options.userId;
        ...
        if ((options as DisputeListOptions)?.consultationId) where.consultationId = (options as DisputeListOptions).consultationId;
        if ((options as DisputeListOptions)?.legalOpinionId) where.legalOpinionId = (options as DisputeListOptions).legalOpinionId;
        if ((options as DisputeListOptions)?.serviceRequestId) where.serviceRequestId = (options as DisputeListOptions).serviceRequestId;
        if ((options as DisputeListOptions)?.litigationCaseId) where.litigationCaseId = (options as DisputeListOptions).litigationCaseId;
        if (options?.fromDate || options?.toDate) {
            where.createdAt = {};
            if (options?.fromDate) where.createdAt.gte = options.fromDate;
            if (options?.toDate) where.createdAt.lte = options.toDate;
 ...
 */
class TransactionalTransactionLogRepository implements ITransactionLogRepository {
    constructor(private readonly prisma: PrismaService | PrismaTransactionClient) {}
    ...
    async findById(id: string): Promise<TransactionLog | null> {
        const found = await this.prisma.transactionLog.findUnique({ where: { id } });
        return found ? this.toDomain(found) : null;
    }

    async findByReference(reference: string): Promise<TransactionLog | null> {
        const found = await this.prisma.transactionLog.findFirst({ where: { reference } });
        return found ? this.toDomain(found) : null;
    }
    ...
    async count(options?: TransactionLogCountOptions): Promise<number> {
        return await this.prisma.transactionLog.count({ where: this.buildWhereClause(options) });
    }
    ...
    async markAsRefunded(id: string): Promise<TransactionLog> {
        const updated = await this.prisma.transactionLog.update({
            where: { id },
            data: { status: PaymentStatusMapper.toPrisma(PaymentStatusEnum.REFUNDED) },
        });
        return this.toDomain(updated);
    }
    ...
    async markAsPartiallyRefunded(id: string): Promise<TransactionLog> {
        const updated = await this.prisma.transactionLog.update({
            where: { id },
            data: { status: PaymentStatusMapper.toPrisma(PaymentStatusEnum.PARTIALLY_REFUNDED) },
        });
        return this.toDomain(updated);
    }
    ...
    async findSuccessfulTransactions(userId: string, fromDate?: Date, toDate?: Date): Promise<TransactionLog[]> {
        const found = await this.prisma.transactionLog.findMany({
            where: {
                userId,
    ...
    async getUserTransactionSummary(userId: string, fromDate?: Date, toDate?: Date): Promise<TransactionSummary> {
        const dateFilter = fromDate || toDate ? {
        ...
        const [credits, debits, count] = await Promise.all([
            ...
            this.prisma.transactionLog.count({
                where: { userId, status: PaymentStatusMapper.toPrisma(PaymentStatusEnum.PAID), ...dateFilter },
            }),
        ]);

        const totalCredits = credits._sum.amount ?? 0;
        const totalDebits = debits._sum.amount ?? 0;
        return { totalCredits, totalDebits, netAmount: totalCredits - totalDebits, transactionCount: count };
    }

    async getTotalSpent(userId: string, fromDate?: Date, toDate?: Date): Promise<number> {
    ...
    async getTotalRefunded(userId: string, fromDate?: Date, toDate?: Date): Promise<number> {
        const result = await this.prisma.transactionLog.aggregate({
            where: {
                userId,
                status: PaymentStatusMapper.toPrisma(PaymentStatusEnum.PAID),
    ...
    private buildWhereClause(options?: TransactionLogListOptions | TransactionLogCountOptions): any {
        const where: any = {};
        if (options?.userId) where.userId = options.userId;
        if (options?.type) where.type = TransactionTypeMapper.toPrisma(options.type);
        if (options?.status) where.status = PaymentStatusMapper.toPrisma(options.status);
        if (options?.fromDate || options?.toDate) {
            where.createdAt = {};
            if (options?.fromDate) where.createdAt.gte = options.fromDate;
            if (options?.toDate) where.createdAt.lte = options.toDate;
 ...
 */
class TransactionalMembershipInvoiceRepository implements IMembershipInvoiceRepository {
    constructor(private readonly prisma: PrismaService | PrismaTransactionClient) {}
    ...
    async findById(id: string): Promise<MembershipInvoice | null> {
        const found = await this.prisma.membershipInvoice.findUnique({ where: { id } });
        return found ? this.toDomain(found) : null;
    }

    async findByInvoiceNumber(invoiceNumber: string): Promise<MembershipInvoice | null> {
        const found = await this.prisma.membershipInvoice.findUnique({ where: { invoiceNumber } });
        return found ? this.toDomain(found) : null;
    }
    ...
    async count(options?: MembershipInvoiceCountOptions): Promise<number> {
        return await this.prisma.membershipInvoice.count({ where: this.buildWhereClause(options) });
    }
    ...
    private buildWhereClause(options?: MembershipInvoiceListOptions | MembershipInvoiceCountOptions): any {
        const where: any = {};
        if (options?.membershipId) where.membershipId = options.membershipId;
        if (options?.status) where.status = InvoiceStatusMapper.toPrisma(options.status);
        if (options?.dueBefore) where.dueDate = { ...where.dueDate, lte: options.dueBefore };
        if (options?.dueAfter) where.dueDate = { ...where.dueDate, gte: options.dueAfter };
        return where;
    }
