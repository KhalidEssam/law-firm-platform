...
export class PrismaNotificationRepository implements INotificationRepository {
    ...
    async getRecentByUserId(userId: string, limit: number = 10): Promise<Notification[]> {
        const found = await this.prisma.notification.findMany({
            where: { userId },
            take: limit,
            orderBy: { createdAt: 'desc' },
    ...
    // PRIVATE HELPERS
    // ============================================
    private buildWhereClause(options?: NotificationListOptions | NotificationCountOptions) {
        const where: any = {};

        if (options?.userId) where.userId = options.userId;
        if (options?.type) where.type = options.type;
        if (options?.isRead !== undefined) where.isRead = options.isRead;
        if (options?.relatedEntityType) where.relatedEntityType = options.relatedEntityType;
        if ((options as NotificationListOptions)?.relatedEntityId) {
            where.relatedEntityId = (options as NotificationListOptions).relatedEntityId;
        }
