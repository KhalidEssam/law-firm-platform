...
export class PrismaNotificationPreferenceRepository implements INotificationPreferenceRepository {
    ...
    async createMany(preferences: NotificationPreference[]): Promise<NotificationPreference[]> {
    ...
    async getEnabledChannelsForEvent(userId: string, eventType: string): Promise<NotificationChannel[]> {
        ...
        const enabledChannels = prefs.map(p => NotificationChannelMapper.toDomain(p.channel));
        ...
        for (const channel of allChannels) {
            ...
            if (!hasPreference) {
                ...
                if (channel === NotificationChannel.EMAIL || channel === NotificationChannel.IN_APP) {
