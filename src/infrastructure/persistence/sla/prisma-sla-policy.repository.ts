...
export class PrismaSLAPolicyRepository implements ISLAPolicyRepository {
    ...
    async findByTypeAndPriority(requestType: RequestType, priority: Priority): Promise<SLAPolicy | null> {
    ...
    async findBestMatch(requestType: RequestType, priority?: Priority): Promise<SLAPolicy | null> {
        // First try exact match
        if (priority) {
            const exactMatch = await this.findByTypeAndPriority(requestType, priority);
            if (exactMatch) return exactMatch;
        }

        // Fall back to normal priority
        const normalMatch = await this.findByTypeAndPriority(requestType, Priority.NORMAL);
    ...
    async existsByTypeAndPriority(requestType: RequestType, priority: Priority): Promise<boolean> {
        const count = await this.prisma.sLAPolicy.count({
