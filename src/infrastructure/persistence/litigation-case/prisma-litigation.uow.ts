...
class CaseStatusMapper {
    ...
    static toPrismaString(status: string): string {
        ...
        return this.toPrismaMap[caseStatus.getValue() as CaseStatusEnum] || status;
    }
}
...
export class PrismaLitigationUnitOfWork implements ILitigationUnitOfWork {
    ...
    constructor(private readonly prisma: PrismaService) {
        ...
        this._statusHistories = new TransactionalLitigationStatusHistoryRepository(prisma);
    }
     ...
     */
    private createTransactionalUow(tx: PrismaTransactionClient): ILitigationUnitOfWork {
        return {
            cases: new TransactionalLitigationCaseRepository(tx),
            statusHistories: new TransactionalLitigationStatusHistoryRepository(tx),
            transaction: async <R>(work: (uow: ILitigationUnitOfWork) => Promise<R>): Promise<R> => {
...
class TransactionalLitigationCaseRepository implements ILitigationCaseRepository {
    ...
    private buildWhereClause(filters?: LitigationCaseFilters): Prisma.LitigationCaseWhereInput {
    ...
    private toPrisma(litigationCase: LitigationCase): Prisma.LitigationCaseCreateInput {
        return {
            ...
            caseDetails: litigationCase.caseDetails?.toJSON() as Prisma.InputJsonValue || Prisma.JsonNull,
            status: CaseStatusMapper.toPrisma(litigationCase.status),
            quoteAmount: litigationCase.quoteAmount?.getAmount() || null,
            quoteCurrency: (litigationCase.quoteAmount?.getCurrency() as PrismaCurrency) || null,
            quoteDetails: litigationCase.quoteDetails?.toJSON() as Prisma.InputJsonValue || Prisma.JsonNull,
    ...
    private toUpdatePrisma(litigationCase: LitigationCase): Prisma.LitigationCaseUpdateInput {
        return {
            assignedProvider: litigationCase.assignedProviderId
                ? { connect: { id: litigationCase.assignedProviderId.getValue() } }
                : undefined,
...
class TransactionalLitigationStatusHistoryRepository implements ILitigationStatusHistoryRepository {
    constructor(private readonly prisma: PrismaService | PrismaTransactionClient) {}

    async create(history: LitigationStatusHistory): Promise<LitigationStatusHistory> {
    ...
    async createMany(histories: LitigationStatusHistory[]): Promise<LitigationStatusHistory[]> {
    ...
    async findLatestByLitigationCaseId(litigationCaseId: CaseId): Promise<LitigationStatusHistory | null> {
