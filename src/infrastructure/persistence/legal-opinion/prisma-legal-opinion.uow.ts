...
import {
  OpinionStatusVO,
  OpinionStatus,
} from '../../../core/domain/legal-opinion/value-objects/opinion-status.vo';
import {
  OpinionTypeVO,
  OpinionType,
} from '../../../core/domain/legal-opinion/value-objects/opinion-type.vo';
import {
  OpinionPriorityVO,
  OpinionPriority,
} from '../../../core/domain/legal-opinion/value-objects/opinion-priority.vo';
import {
  DeliveryFormatVO,
  DeliveryFormat,
} from '../../../core/domain/legal-opinion/value-objects/delivery-format.vo';
...
export class PrismaLegalOpinionUnitOfWork implements ILegalOpinionUnitOfWork {
  ...
  constructor(private readonly prisma: PrismaService) {
    ...
    this._statusHistories =
      new TransactionalLegalOpinionStatusHistoryRepository(prisma);
  }
...
class TransactionalLegalOpinionRepository
  implements ILegalOpinionRequestRepository
{
  ...
  private toPrisma(opinion: LegalOpinionRequest): any {
    ...
    return {
      ...
      caseDetails,
      status: OpinionStatusMapper.toPrisma(opinion.status.getValue()),
      quoteAmount:
  ...
  private toDomain(data: any): LegalOpinionRequest {
    ...
    return LegalOpinionRequest.reconstitute({
      ...
      draftVersion: 0,
      finalVersion:
        data.status === PrismaRequestStatus.completed ? 1 : undefined,
...
class TransactionalLegalOpinionStatusHistoryRepository
  implements ILegalOpinionStatusHistoryRepository
{
  constructor(
    private readonly prisma: PrismaService | PrismaTransactionClient,
  ) {}
  ...
  async findLatestByLegalOpinionId(
    legalOpinionId: string,
  ): Promise<LegalOpinionStatusHistory | null> {
