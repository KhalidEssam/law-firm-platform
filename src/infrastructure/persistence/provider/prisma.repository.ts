    ...
    return ProviderProfile.create(
      {
        ...
        verificationStatus: VerificationStatusVO.create(
          raw.verificationStatus as VerificationStatus,
        ),
        isActive: raw.isActive,
        workingDays: raw.workingDays
          ? WorkingDays.create(raw.workingDays)
          : undefined,
        workingHours: raw.workingHours
          ? WorkingHours.create(raw.workingHours)
          : undefined,
...
class ProviderServiceMapper {
  static toDomain(raw: any): ProviderService {
    return ProviderService.create(
      {
        providerId: raw.providerId,
        serviceType: ServiceTypeVO.create(
          raw.serviceType as ServiceType,
          raw.category,
        ),
        isActive: raw.isActive,
...
export class PrismaProviderProfileRepository
  implements IProviderProfileRepository
{
  ...
  async list(
    options?: ListProviderProfilesOptions,
  ...
  async findApprovedAndActive(options?: {
    limit?: number;
    offset?: number;
  }): Promise<ProviderProfile[]> {
    const profiles = await this.prisma.providerProfile.findMany({
...
export class PrismaProviderUserRepository implements IProviderUserRepository {
  ...
  async list(options?: ListProviderUsersOptions): Promise<ProviderUser[]> {
    const users = await this.prisma.providerUser.findMany({
      where: {
        ...
        ...(options?.isActive !== undefined && { isActive: options.isActive }),
        ...(options?.canAcceptRequests !== undefined && {
          canAcceptRequests: options.canAcceptRequests,
        }),
        ...(options?.includeDeleted ? {} : { deletedAt: null }),
      },
      take: options?.limit,
  ...
  async findUsersWhoCanAcceptRequests(
    providerId: string,
...
export class PrismaProviderServiceRepository
  implements IProviderServiceRepository
{
  ...
  async findActiveServicesByProvider(
    providerId: string,
  ): Promise<ProviderService[]> {
    const services = await this.prisma.providerService.findMany({
      where: {
...
export class PrismaProviderScheduleRepository
  implements IProviderScheduleRepository
{
  constructor(private readonly prisma: PrismaClient) {}
  ...
  async findAvailableSchedulesByProvider(
    providerId: string,
  ): Promise<ProviderSchedule[]> {
    const schedules = await this.prisma.providerSchedule.findMany({
      where: {
...
export class PrismaUnitOfWork {
  ...
  async transaction<T>(
    work: (uow: PrismaUnitOfWork) => Promise<T>,
  ): Promise<T> {
