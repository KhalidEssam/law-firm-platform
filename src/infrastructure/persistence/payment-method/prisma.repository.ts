...
export class PrismaPaymentMethodRepository implements IPaymentMethodRepository {
    ...
    async getStatistics(filters?: PaymentMethodFilters): Promise<PaymentMethodStatistics> {
        const where: any = {};
    ...
    private toPrisma(paymentMethod: PaymentMethod): any {
        const detailsJson = PaymentMethodDetailsFactory.toJSON(paymentMethod.details);
    ...
    private toDomain(prisma: PrismaPaymentMethod): PaymentMethod {
        ...
        const nickname = detailsJson.nickname ? PaymentNickname.create(detailsJson.nickname) : undefined;
        const isVerified = detailsJson.isVerified ?? false;
        const lastUsedAt = detailsJson.lastUsedAt ? new Date(detailsJson.lastUsedAt) : undefined;
