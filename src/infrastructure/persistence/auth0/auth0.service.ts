...
export class Auth0Service {
    ...
    async getRoleById(roleId: string): Promise<Auth0Role | null> {
        const token = await this.getToken();
        const res = await fetch(`${this.domain}/api/v2/roles/${encodeURIComponent(roleId)}`, {
            headers: { Authorization: `Bearer ${token}` },
        });
    ...
    async getUserRoles(auth0UserId: string): Promise<Auth0Role[]> {
        const token = await this.getToken();
        const res = await fetch(`${this.domain}/api/v2/users/${encodeURIComponent(auth0UserId)}/roles`, {
            headers: { Authorization: `Bearer ${token}` },
        });

        if (!res.ok) {
            const text = await res.text();
            throw new Error(`Failed to fetch user roles: ${res.statusText} → ${text}`);
        }
    ...
    async getUser(auth0UserId: string): Promise<Auth0User | null> {
        const token = await this.getToken();
        const res = await fetch(`${this.domain}/api/v2/users/${encodeURIComponent(auth0UserId)}`, {
            headers: { Authorization: `Bearer ${token}` },
        });
     ...
     */
    async createUser(email: string, password: string, username: string): Promise<Auth0User> {
        const token = await this.getToken();
     ...
     */
    async getRolePermissions(roleId: string): Promise<{ permission_name: string; description: string }[]> {
        const token = await this.getToken();
        const res = await fetch(`${this.domain}/api/v2/roles/${encodeURIComponent(roleId)}/permissions`, {
            headers: { Authorization: `Bearer ${token}` },
        });

        if (!res.ok) {
            const text = await res.text();
            throw new Error(`Failed to fetch role permissions: ${res.statusText} → ${text}`);
        }
     ...
     */
    async getUserPermissions(auth0UserId: string): Promise<{ permission_name: string; description: string }[]> {
        const token = await this.getToken();
        const res = await fetch(`${this.domain}/api/v2/users/${encodeURIComponent(auth0UserId)}/permissions`, {
            headers: { Authorization: `Bearer ${token}` },
        });

        if (!res.ok) {
            const text = await res.text();
            throw new Error(`Failed to fetch user permissions: ${res.statusText} → ${text}`);
        }
