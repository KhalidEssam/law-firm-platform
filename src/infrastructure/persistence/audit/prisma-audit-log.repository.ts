 ...
 */
function mapEntityTypeToPrisma(entityType: AuditEntityType | undefined): RequestType | undefined {
 ...
 */
function mapPrismaToEntityType(requestType: RequestType | null): AuditEntityType | null {
...
export class PrismaAuditLogRepository implements IAuditLogRepository {
    ...
    async findByUserId(userId: string, options?: AuditLogPaginationOptions): Promise<AuditLog[]> {
    ...
    async findByAction(action: string, options?: AuditLogPaginationOptions): Promise<AuditLog[]> {
        const records = await this.prisma.auditLog.findMany({
            where: { action },
            orderBy: { [options?.orderBy || 'createdAt']: options?.orderDir || 'desc' },
            take: options?.limit || 50,
    ...
    async getStatistics(filters?: AuditLogFilters): Promise<AuditLogStatistics> {
        ...
        const [totalCount, byAction, byEntityType, byUser, recentActivity] = await Promise.all([
            this.prisma.auditLog.count({ where }),
    ...
    private async getRecentActivityByDay(where: any): Promise<{ date: string; count: number }[]> {
