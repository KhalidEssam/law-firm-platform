...
export class ConsultationRequestRepository {
  ...
  async create(
    ...
    const data: Prisma.ConsultationRequestCreateInput = {
      ...
      assignedProvider: consultation.assignedProviderId
        ...
        : undefined,
      consultationType: ConsultationTypeMapper.toPrisma(
  ...
  async getStatistics(filters?: ConsultationFilters): Promise<{
    ...
    const byUrgency = urgencyGroups.reduce(
      (acc, group) => {
    ...
    const bySLAStatus = slaGroups.reduce(
      (acc, group) => {
        if (group.slaStatus) {
    ...
    const averageResponseTime =
      assignedRecords.length > 0
        ? assignedRecords.reduce((sum, record) => {
            const diff =
              record.assignedAt!.getTime() - record.submittedAt.getTime();
    ...
    const averageCompletionTime =
      completedRecords.length > 0
        ? completedRecords.reduce((sum, record) => {
            const diff =
              record.completedAt!.getTime() - record.submittedAt.getTime();
            return sum + diff / (1000 * 60 * 60);
