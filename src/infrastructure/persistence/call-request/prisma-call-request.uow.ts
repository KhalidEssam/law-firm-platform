...
export class PrismaCallRequestUnitOfWork implements ICallRequestUnitOfWork {
    ...
    constructor(private readonly prisma: PrismaService) {
        ...
        this._statusHistories = new TransactionalCallStatusHistoryRepository(prisma);
    }
     ...
     */
    private createTransactionalUow(tx: PrismaTransactionClient): ICallRequestUnitOfWork {
        return {
            callRequests: new TransactionalCallRequestRepository(tx),
            statusHistories: new TransactionalCallStatusHistoryRepository(tx),
            transaction: async <R>(work: (uow: ICallRequestUnitOfWork) => Promise<R>): Promise<R> => {
...
class TransactionalCallRequestRepository implements ICallRequestRepository {
    ...
    async findByRequestNumber(requestNumber: string): Promise<CallRequest | null> {
        const data = await this.prisma.callRequest.findUnique({
    ...
    async findAll(
        ...
        if (filter?.assignedProviderId) where.assignedProviderId = filter.assignedProviderId;
        if (filter?.consultationType) where.consultationType = filter.consultationType;

        if (filter?.status) {
            if (Array.isArray(filter.status)) {
                where.status = { in: filter.status.map(s => this.mapStatusToPrisma(s)) };
            } else {
        ...
        if (filter?.scheduledAfter || filter?.scheduledBefore) {
            ...
            if (filter.scheduledBefore) where.scheduledAt.lte = filter.scheduledBefore;
        }
    ...
    async countBySubscriber(subscriberId: string, status?: CallStatus): Promise<number> {
        const where: Prisma.CallRequestWhereInput = {
            subscriberId,
            deletedAt: null,
        };
    ...
    async countByProvider(providerId: string, status?: CallStatus): Promise<number> {
        const where: Prisma.CallRequestWhereInput = {
            assignedProviderId: providerId,
            deletedAt: null,
        };
    ...
    async findConflictingCalls(
        ...
        return data
            .filter(d => {
                ...
                const existingEnd = new Date(d.scheduledAt.getTime() + d.scheduledDuration * 60000);
                // Check for overlap
    ...
    async isProviderAvailable(
        ...
        const conflicts = await this.findConflictingCalls(providerId, scheduledAt, durationMinutes);
...
class TransactionalCallStatusHistoryRepository implements ICallStatusHistoryRepository {
    constructor(private readonly prisma: PrismaService | PrismaTransactionClient) {}
    ...
    async createMany(histories: CallStatusHistory[]): Promise<CallStatusHistory[]> {
    ...
    async findLatestByCallRequestId(callRequestId: string): Promise<CallStatusHistory | null> {
