...
import { SupportTicket, AssignTicketData } from '../../../core/domain/support-ticket/entities/support-ticket.entity';
...
export class PrismaSupportTicketRepository implements ISupportTicketRepository {
    ...
    async create(ticket: SupportTicket): Promise<SupportTicket> {
        const created = await this.prisma.supportTicket.create({
            data: {
                ...
                priority: SupportTicketPriorityMapper.toPrisma(ticket.priority.getValue()),
    ...
    async assign(id: string, assignData: AssignTicketData): Promise<SupportTicket> {
        const updated = await this.prisma.supportTicket.update({
            where: { id },
            data: {
    ...
    async updatePriority(id: string, priority: PriorityEnum): Promise<SupportTicket> {
        const updated = await this.prisma.supportTicket.update({
            where: { id },
            data: {
    ...
    async getStatistics(fromDate?: Date, toDate?: Date): Promise<SupportTicketStatistics> {
        ...
        const [
            ...
            this.prisma.supportTicket.count({
                where: { status: TicketStatusMapper.toPrisma(TicketStatusEnum.OPEN), ...baseFilter },
            }),
            this.prisma.supportTicket.count({
                where: { status: TicketStatusMapper.toPrisma(TicketStatusEnum.IN_PROGRESS), ...baseFilter },
            }),
            this.prisma.supportTicket.count({
                where: { status: TicketStatusMapper.toPrisma(TicketStatusEnum.RESOLVED), ...baseFilter },
            }),
            this.prisma.supportTicket.count({
                where: { status: TicketStatusMapper.toPrisma(TicketStatusEnum.CLOSED), ...baseFilter },
            }),
            Promise.all([
                this.prisma.supportTicket.count({ where: { category: TicketCategoryMapper.toPrisma(TicketCategoryEnum.TECHNICAL), ...baseFilter } }),
                this.prisma.supportTicket.count({ where: { category: TicketCategoryMapper.toPrisma(TicketCategoryEnum.BILLING), ...baseFilter } }),
                this.prisma.supportTicket.count({ where: { category: TicketCategoryMapper.toPrisma(TicketCategoryEnum.GENERAL), ...baseFilter } }),
                this.prisma.supportTicket.count({ where: { category: TicketCategoryMapper.toPrisma(TicketCategoryEnum.COMPLAINT), ...baseFilter } }),
            ]),
            Promise.all([
                this.prisma.supportTicket.count({ where: { priority: SupportTicketPriorityMapper.toPrisma(PriorityEnum.LOW), ...baseFilter } }),
                this.prisma.supportTicket.count({ where: { priority: SupportTicketPriorityMapper.toPrisma(PriorityEnum.NORMAL), ...baseFilter } }),
                this.prisma.supportTicket.count({ where: { priority: SupportTicketPriorityMapper.toPrisma(PriorityEnum.HIGH), ...baseFilter } }),
                this.prisma.supportTicket.count({ where: { priority: SupportTicketPriorityMapper.toPrisma(PriorityEnum.URGENT), ...baseFilter } }),
            ]),
            this.prisma.supportTicket.count({
                where: {
                    assignedTo: null,
                    status: { in: [TicketStatusMapper.toPrisma(TicketStatusEnum.OPEN), TicketStatusMapper.toPrisma(TicketStatusEnum.IN_PROGRESS)] },
                    ...baseFilter,
                },
            }),
            this.prisma.supportTicket.count({
    ...
    // PRIVATE HELPERS
    // ============================================
    private buildWhereClause(options?: SupportTicketListOptions | SupportTicketCountOptions) {
        const where: any = { deletedAt: null };

        if (options?.subscriberId) where.subscriberId = options.subscriberId;
        if (options?.status) where.status = TicketStatusMapper.toPrisma(options.status);
        if (options?.category) where.category = TicketCategoryMapper.toPrisma(options.category);
        if (options?.priority) where.priority = SupportTicketPriorityMapper.toPrisma(options.priority);
