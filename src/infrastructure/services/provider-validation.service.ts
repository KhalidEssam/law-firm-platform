...
export class ProviderValidationService implements IProviderValidationService {
    ...
    async validateProviderForAssignment(userId: string): Promise<ProviderValidationResult> {
        ...
        if (providerUsers.length === 0) {
            return {
                isValid: false,
                error: `User ${userId} is not linked to any service provider organization. ` +
        ...
        if (!activeProviderUser) {
            return {
                isValid: false,
                error: `User ${userId} has a provider association but it is currently inactive. ` +
        ...
        if (!activeProviderUser.canAcceptRequests) {
            return {
                isValid: false,
                error: `User ${userId} is linked to a provider organization but is not authorized to accept service requests. ` +
                    `The provider administrator needs to enable request acceptance for this user.`,
            };
        }

        // Step 3: Get and validate the ProviderProfile
        const providerProfile = await this.providerProfileRepo.findById(activeProviderUser.providerId);

        if (!providerProfile) {
            return {
                isValid: false,
                error: `Provider profile not found for provider ID ${activeProviderUser.providerId}. ` +
        ...
        if (!providerProfile.isApproved) {
            return {
                isValid: false,
                error: `The service provider organization "${providerProfile.organizationName.name}" ` +
    ...
    async getValidatedProviderInfo(userId: string): Promise<ValidatedProviderInfo | null> {
