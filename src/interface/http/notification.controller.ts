...
export class NotificationController {
  ...
  async sendTemplatedNotification(@Body() dto: SendTemplatedNotificationDto) {
    const notification =
      await this.sendTemplatedNotificationUseCase.execute(dto);
    return {
      success: true,
      data: this.mapNotification(notification),
    };
  ...
  async getUnreadNotifications(
    ...
    const notifications = await this.getNotificationsUseCase.unreadForUser(
      userId,
      {
        limit: limit ? parseInt(limit) : 50,
      },
  ...
  async renderTemplate(
    ...
    const rendered = await this.renderTemplateUseCase.preview(
      id,
      dto.variables,
    );
    return {
  ...
  async getUserChannelPreferences(
    ...
    const prefs = await this.getPreferencesUseCase.forUserAndChannel(
      userId,
      channel,
    );
    return {
  ...
  async checkPreference(
    ...
    const enabled = await this.getPreferencesUseCase.isEnabled(
      userId,
      channel,
      eventType,
    );
  ...
  async getEnabledChannels(
    ...
    const channels =
      await this.getPreferencesUseCase.getEnabledChannelsForEvent(
        userId,
        eventType,
      );
    return {
  ...
  async bulkSetPreferences(
    @Param('userId') userId: string,
    @Body() dto: BulkSetPreferencesDto,
  ) {
