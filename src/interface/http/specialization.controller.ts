...
export class SpecializationController {
    ...
    async findByCategory(@Param('category') category: string) {
        const specializations = await this.listSpecializations.executeByCategory(category);
        return {
            data: specializations.map(spec => ({
                id: spec.id,
                name: spec.name,
    ...
    async update(@Param('id') id: string, @Body() dto: Omit<UpdateSpecializationDTO, 'id'>) {
    ...
    async activate(@Param('id') id: string) {
        const spec = await this.updateSpecialization.execute({ id, isActive: true });
    ...
    async deactivate(@Param('id') id: string) {
        const spec = await this.updateSpecialization.execute({ id, isActive: false });
        return { id: spec.id, isActive: spec.isActive };
    }
}
...
export class ProviderSpecializationController {
    ...
    async findAll(
        ...
        const result = await this.listProviderSpecializations.execute({
            providerId,
            specializationId,
            isCertified: isCertified !== undefined ? isCertified === 'true' : undefined,
            minExperienceYears: minExperienceYears ? parseInt(minExperienceYears, 10) : undefined,
            limit: limit ? parseInt(limit, 10) : undefined,
            offset: offset ? parseInt(offset, 10) : undefined,
        });
    ...
    async findByProvider(@Param('providerId') providerId: string) {
        const specializations = await this.listProviderSpecializations.executeByProvider(providerId);
        return {
    ...
    async getProviderExpertise(@Param('providerId') providerId: string) {
        const expertise = await this.listProviderSpecializations.getProviderExpertise(providerId);
        return {
    ...
    async findCertifiedByProvider(@Param('providerId') providerId: string) {
        const specializations = await this.listProviderSpecializations.executeCertifiedByProvider(providerId);
        return {
            data: specializations.map(ps => ({
                id: ps.id,
                specializationId: ps.specializationId,
    ...
    async findTopProviders(
        ...
        const providers = await this.listProviderSpecializations.findTopProvidersBySpecialization(
            specializationId,
    ...
    async countProviders(@Param('specializationId') specializationId: string) {
        const count = await this.listProviderSpecializations.countProvidersBySpecialization(specializationId);
        return { count };
    }
    ...
    async update(@Param('id') id: string, @Body() dto: Omit<UpdateProviderSpecializationDTO, 'id'>) {
    ...
    async addCertification(
        ...
        const ps = await this.updateProviderSpecialization.addCertification(id, certification);
        return {
            id: ps.id,
            isCertified: ps.isCertified,
            certifications: ps.certifications,
    ...
    async removeCertification(
        ...
        const ps = await this.updateProviderSpecialization.removeCertification(id, certificationName);
        return {
            id: ps.id,
            isCertified: ps.isCertified,
            certifications: ps.certifications,
