...
export class CallRequestController {
  ...
  constructor(
    ...
    @Optional()
    @Inject(CALL_NOTIFICATION_SERVICE)
  ...
  async findAll(
    @Query() query: GetCallRequestsQueryDto,
  ): Promise<CallRequestListResponseDto> {
  ...
  async assign(
    ...
    this.sendNotificationSafe(() =>
      this.notificationService?.notifyCallAssigned(
        callRequest.id,
        dto.providerId,
      ),
    );
  ...
  async checkAvailability(
    ...
    return {
      ...
      conflictingCalls: result.conflictingCalls.map((cr) =>
        this.mapToResponse(cr),
      ),
    };
  }
   ...
   */
  private sendNotificationSafe(
    notificationFn: () => Promise<void> | undefined,
  ): void {
