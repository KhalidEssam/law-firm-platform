...
export class PaymentMethodController {
    ...
    @ApiResponse({ status: 404, description: 'Payment method not found' })
    @ApiResponse({ status: 400, description: 'Cannot set as default - not verified' })
    async setAsDefault(
        @Param('id') id: string,
        @Req() req: any,
    ): Promise<any> {
    ...
    async listPaymentMethods(@Query() query: ListPaymentMethodsDto): Promise<any> {
    ...
    @ApiResponse({ status: 200, description: 'Expiring payment methods retrieved' })
    ...
    @ApiResponse({ status: 204, description: 'Payment method permanently deleted' })
    @ApiResponse({ status: 404, description: 'Payment method not found' })
