...
export class MembershipController {
    ...
    async createNewTierService(
        ...
        @Body() dto: { serviceId: string; quotaPerMonth?: number; quotaPerYear?: number; rolloverUnused?: boolean; discountPercent?: number },
    ...
    async getTierServices(@Param('tierId') tierId: string): Promise<{ tierServices: any[] }> {
        const tierServices = await this.getTierServicesByTierId.execute(parseInt(tierId));
        return { tierServices };
    }
    ...
    async updateTierServiceById(
        @Param('id') id: string,
        @Body() dto: { quotaPerMonth?: number; quotaPerYear?: number; rolloverUnused?: boolean; discountPercent?: number; isActive?: boolean },
    ): Promise<{ tierService: any }> {
    ...
    @HttpCode(HttpStatus.OK)
    async deleteTierServiceById(@Param('id') id: string): Promise<{ message: string }> {
    ...
    async checkTierServiceQuota(
        ...
        const quota = await this.checkServiceQuota.execute(parseInt(tierId), serviceId);
        return { quota };
    }
    ...
    @Roles('user', 'system admin')
    async getUsageSummary(@Param('id') membershipId: string): Promise<{ summary: any[] }> {
    ...
    async checkRemainingQuota(
        ...
        const quota = await this.checkRemainingServiceQuota.execute(membershipId, serviceId);
        return { quota };
    }
    ...
    @Roles('platform', 'system admin')
    async getUnbilledServiceUsage(@Param('id') membershipId: string): Promise<{ usage: any[] }> {
    ...
    async markAsBilled(
        ...
        const usage = await this.markUsageAsBilled.execute(usageId, dto.amount, dto.currency || 'SAR');
        return { usage };
    }
    ...
    async upgrade(
        ...
        const result = await this.upgradeMembership.execute(membershipId, dto.newTierId, req.user?.userId);
        return { result };
    }
    ...
    async downgrade(
        ...
        const result = await this.downgradeMembership.execute(membershipId, dto.newTierId, req.user?.userId);
        return { result };
    }
    ...
    async getChangeHistory(
        ...
        const history = await this.getMembershipChangeHistory.execute(membershipId, {
            limit: limit ? parseInt(limit) : undefined,
            offset: offset ? parseInt(offset) : undefined,
        });
        return { history };
    ...
    @Get(':id/status')
    @Roles('user', 'system admin')
    async getMembershipStatus(@Param('id') membershipId: string): Promise<{ status: any }> {
    ...
    async listAllMemberships(
        ...
        return {
            memberships: result.memberships.map((m) => this.mapMembershipToResponse(m)),
            total: result.total,
        };
    }
