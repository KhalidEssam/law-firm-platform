...
export interface ITierServiceRepository {
    ...
    findByTierAndService(tierId: number, serviceId: string): Promise<TierService | null>;

    /** Find all services for a tier */
    findByTierId(tierId: number, options?: { isActive?: boolean }): Promise<TierService[]>;

    /** Find all tiers for a service */
    findByServiceId(serviceId: string, options?: { isActive?: boolean }): Promise<TierService[]>;
    ...
    getQuota(tierId: number, serviceId: string): Promise<{
