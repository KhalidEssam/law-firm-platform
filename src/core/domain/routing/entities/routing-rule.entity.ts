...
import { RequestType, isValidRequestType } from '../value-objects/request-type.vo';
...
export class RoutingRule {
    ...
    static create(props: RoutingRuleProps): RoutingRule {
        ...
        const routingStrategy = typeof props.routingStrategy === 'string'
            ? (isValidRoutingStrategy(props.routingStrategy) ? props.routingStrategy as RoutingStrategy : RoutingStrategy.MANUAL)
    ...
    private selectRoundRobin(
        ...
        const nextIndex = lastAssignedIndex !== undefined
    ...
    private selectSpecialized(providers: ProviderInfo[]): ProviderInfo {
        ...
        const matchingProviders = providers.filter(p => {
            ...
            return requiredSpecs.some(req => providerSpecs.includes(req.toLowerCase()));
        });
