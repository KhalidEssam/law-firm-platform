...
import { TicketCategory, TicketCategoryEnum } from '../value-objects/ticket-category.vo';
import { Priority, PriorityEnum } from '../../billing/value-objects/priority.vo';
...
export class SupportTicket {
    ...
    assign(assignData: AssignTicketData): SupportTicket {
        if (!this.status.canBeAssigned()) {
            throw new Error(`Cannot assign ticket. Current status: ${this.status.getValue()}`);
        }
        return new SupportTicket(
            this.id,
            this.ticketNumber,
    ...
    startProgress(): SupportTicket {
        if (!this.status.canStartProgress()) {
            throw new Error(`Cannot start progress. Current status: ${this.status.getValue()}`);
        }
        return new SupportTicket(
            this.id,
            this.ticketNumber,
    ...
    resolve(): SupportTicket {
        if (!this.status.canBeResolved()) {
            throw new Error(`Cannot resolve ticket. Current status: ${this.status.getValue()}`);
        }
        return new SupportTicket(
            this.id,
            this.ticketNumber,
    ...
    close(): SupportTicket {
        if (!this.status.canBeClosed()) {
            throw new Error(`Cannot close ticket. Current status: ${this.status.getValue()}`);
        }
        return new SupportTicket(
            this.id,
            this.ticketNumber,
    ...
    reopen(): SupportTicket {
        if (!this.status.canBeReopened()) {
            throw new Error(`Cannot reopen ticket. Current status: ${this.status.getValue()}`);
        }
        return new SupportTicket(
            this.id,
            this.ticketNumber,
