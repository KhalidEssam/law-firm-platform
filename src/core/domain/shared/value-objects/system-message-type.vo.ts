...
export class SystemMessageType {
    ...
    isDismissibleByDefault(): boolean {
        ...
        return [SystemMessageTypeEnum.BANNER, SystemMessageTypeEnum.ALERT].includes(this.value);
    }
 ...
 */
export function isValidSystemMessageType(value: string): value is SystemMessageTypeEnum {
    return Object.values(SystemMessageTypeEnum).includes(value as SystemMessageTypeEnum);
}

/**
 * Map Prisma enum to domain SystemMessageType
 */
export function mapSystemMessageTypeFromPrisma(prismaType: string): SystemMessageType {
