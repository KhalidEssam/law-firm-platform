...
export class SLADeadlines {
  ...
  static calculate(
    ...
    const responseDeadline = new Date(
      startDate.getTime() + times.responseTime * 60 * 1000,
    );
    const resolutionDeadline = new Date(
      startDate.getTime() + times.resolutionTime * 60 * 1000,
    );
  ...
  private isResponseAtRisk(now: Date): boolean {
    const totalTime =
      this.responseDeadline.getTime() - this.createdAt.getTime();
  ...
  private isResolutionAtRisk(now: Date): boolean {
    const totalTime =
      this.resolutionDeadline.getTime() - this.createdAt.getTime();
    const elapsedTime = now.getTime() - this.createdAt.getTime();
    return elapsedTime / totalTime >= AT_RISK_THRESHOLD;
  }
