...
import { RequestType, DEFAULT_SLA_TIMES } from '../value-objects/request-type.vo';
...
export class SLACalculatorService {
    ...
    getRequestSLAInfo(
        ...
        const overallStatus = getMostSevereStatus([responseStatus, resolutionStatus]);
    ...
    isAtRisk(
        ...
        return {
            response: !responded && deadlines.getResponseTimeElapsedPercent(now) >= threshold,
            resolution: !resolved && deadlines.getResolutionTimeElapsedPercent(now) >= threshold,
        };
    }
    ...
    calculateAtRiskTime(
        ...
        const responseTotal = deadlines.responseDeadline.getTime() - deadlines.createdAt.getTime();
        const resolutionTotal = deadlines.resolutionDeadline.getTime() - deadlines.createdAt.getTime();

        return {
            response: new Date(deadlines.createdAt.getTime() + (responseTotal * threshold / 100)),
            resolution: new Date(deadlines.createdAt.getTime() + (resolutionTotal * threshold / 100)),
        };
    }
