...
import { TransactionType, TransactionTypeEnum } from '../value-objects/transaction-type.vo';
...
export class TransactionLog {
    ...
    markAsPaid(): TransactionLog {
        if (!this.status.canBeRetried()) {
            throw new Error(`Cannot mark transaction as paid. Current status: ${this.status.getValue()}`);
        }
        return new TransactionLog(
            this.id,
            this.userId,
    ...
    markAsFailed(): TransactionLog {
        if (this.status.isFinal()) {
            throw new Error(`Cannot mark transaction as failed. Current status: ${this.status.getValue()}`);
        }
        return new TransactionLog(
            this.id,
            this.userId,
    ...
    markAsRefunded(): TransactionLog {
        if (!this.status.canBeRefunded()) {
            throw new Error(`Cannot mark transaction as refunded. Current status: ${this.status.getValue()}`);
        }
        return new TransactionLog(
            this.id,
            this.userId,
    ...
    markAsPartiallyRefunded(): TransactionLog {
        if (!this.status.canBeRefunded()) {
            throw new Error(`Cannot mark transaction as partially refunded. Current status: ${this.status.getValue()}`);
        }
        return new TransactionLog(
            this.id,
            this.userId,
