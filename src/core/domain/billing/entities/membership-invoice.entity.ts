...
import { InvoiceStatus, InvoiceStatusEnum } from '../value-objects/invoice-status.vo';
...
export class MembershipInvoice {
    ...
    markAsPaid(): MembershipInvoice {
        if (!this.status.canBePaid()) {
            throw new Error(`Cannot mark invoice as paid. Current status: ${this.status.getValue()}`);
        }
        return new MembershipInvoice(
            this.id,
            this.membershipId,
    ...
    markAsOverdue(): MembershipInvoice {
        if (!this.status.canBeMarkedOverdue()) {
            throw new Error(`Cannot mark invoice as overdue. Current status: ${this.status.getValue()}`);
        }
        return new MembershipInvoice(
            this.id,
            this.membershipId,
    ...
    cancel(): MembershipInvoice {
        if (!this.status.canBeCancelled()) {
            throw new Error(`Cannot cancel invoice. Current status: ${this.status.getValue()}`);
        }
        return new MembershipInvoice(
            this.id,
            this.membershipId,
