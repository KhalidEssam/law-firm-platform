...
import { RefundStatus, RefundStatusEnum } from '../value-objects/refund-status.vo';
...
export class Refund {
    ...
    approve(reviewData: RefundReviewData): Refund {
        if (!this.status.canBeReviewed()) {
            throw new Error(`Cannot approve refund. Current status: ${this.status.getValue()}`);
        }
        return new Refund(
            this.id,
            this.userId,
    ...
    reject(reviewData: RefundReviewData): Refund {
        if (!this.status.canBeReviewed()) {
            throw new Error(`Cannot reject refund. Current status: ${this.status.getValue()}`);
        }
        return new Refund(
            this.id,
            this.userId,
    ...
    process(refundReference: string): Refund {
        if (!this.status.canBeProcessed()) {
            throw new Error(`Cannot process refund. Current status: ${this.status.getValue()}`);
        }
        return new Refund(
            this.id,
            this.userId,
