...
import { DisputeStatus, DisputeStatusEnum } from '../value-objects/dispute-status.vo';
...
export class Dispute {
    ...
    startReview(): Dispute {
        if (!this.status.canBeReviewed()) {
            throw new Error(`Cannot start review. Current status: ${this.status.getValue()}`);
        }
        return new Dispute(
            this.id,
            this.userId,
    ...
    escalate(escalationData: DisputeEscalationData): Dispute {
        if (!this.status.canBeEscalated()) {
            throw new Error(`Cannot escalate dispute. Current status: ${this.status.getValue()}`);
        }
        return new Dispute(
            this.id,
            this.userId,
    ...
    resolve(resolutionData: DisputeResolutionData): Dispute {
        if (!this.status.canBeResolved()) {
            throw new Error(`Cannot resolve dispute. Current status: ${this.status.getValue()}`);
        }
        return new Dispute(
            this.id,
            this.userId,
    ...
    close(): Dispute {
        if (!this.status.canBeClosed()) {
            throw new Error(`Cannot close dispute. Current status: ${this.status.getValue()}`);
        }
        return new Dispute(
            this.id,
            this.userId,
