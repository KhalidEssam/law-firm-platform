...
import { CallPlatform, CallPlatformType } from '../value-objects/call-platform.vo';
...
export class CallRequest {
     ...
     */
    static rehydrate(props: Required<Omit<CallRequestProps, 'status'>> & { status: CallStatus }): CallRequest {
        return new CallRequest(
            props.id,
            props.requestNumber,
            props.subscriberId,
    ...
    assignProvider(providerId: string): void {
        if (!canModifyCall(this._status)) {
            throw new Error(`Cannot assign provider when call is in ${this._status} status`);
        }
    ...
    schedule(props: {
        ...
        if (duration.isGreaterThan(MAX_CALL_DURATION)) {
            throw new Error(`Call duration cannot exceed ${MAX_CALL_DURATION.format()}`);
        }
    ...
    reschedule(props: {
        ...
        if (![CallStatus.SCHEDULED, CallStatus.NO_SHOW, CallStatus.RESCHEDULED].includes(this._status)) {
    ...
    startCall(): void {
        if (this._status !== CallStatus.SCHEDULED) {
            throw new Error(`Cannot start call in ${this._status} status. Call must be scheduled first.`);
        }
    ...
    endCall(recordingUrl?: string): void {
        if (this._status !== CallStatus.IN_PROGRESS) {
            throw new Error(`Cannot end call in ${this._status} status. Call must be in progress.`);
        }
    ...
    cancel(reason?: string): void {
        ...
        if (this._status === CallStatus.IN_PROGRESS) {
            throw new Error('Cannot cancel a call that is in progress. End the call instead.');
        }
    ...
    getScheduledDuration(): Duration | null {
        return this._scheduledDuration ? Duration.fromMinutes(this._scheduledDuration) : null;
    }
    ...
    getActualDuration(): Duration | null {
        return this._actualDuration ? Duration.fromMinutes(this._actualDuration) : null;
    }
