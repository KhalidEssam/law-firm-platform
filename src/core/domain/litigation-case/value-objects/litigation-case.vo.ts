...
export class CaseType {
  ...
  static create(value: string): CaseType {
    ...
    if (!this.validTypes.includes(normalizedValue)) {
      throw new Error(
        `Invalid case type: ${value}. Must be one of: ${this.validTypes.join(', ')}`,
      );
    }
...
export class QuoteDetails {
  ...
  private validate(): void {
    if (this.data.contingencyPercentage !== undefined) {
      if (
        this.data.contingencyPercentage < 0 ||
        this.data.contingencyPercentage > 100
      ) {
