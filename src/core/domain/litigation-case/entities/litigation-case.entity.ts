...
export class LitigationCase {
     ...
     */
    static create(props: Omit<LitigationCaseProps, 'id' | 'caseNumber' | 'status' | 'paymentStatus' | 'submittedAt' | 'createdAt' | 'updatedAt'>): LitigationCase {
        const now = new Date();
    ...
    private validate(): void {
        ...
        if (this.props.status.getValue() === 'active' && !this.props.quoteAcceptedAt) {
    ...
    sendQuote(amount: Money, validUntil: Date, details?: QuoteDetails): void {
        if (!this.props.assignedProviderId) {
            throw new Error('Case must have an assigned provider before sending quote');
        }
        ...
        if (!this.props.status.canTransitionTo(CaseStatus.create('quote_sent'))) {
            throw new Error(`Cannot send quote from status: ${this.props.status.getValue()}`);
        }
    ...
    cancel(reason?: string): void {
        ...
        if (this.props.paymentStatus.isPaid()) {
            throw new Error('Cannot cancel case with completed payment. Refund first.');
        }
    ...
    isAwaitingPayment(): boolean {
        return this.props.status.getValue() === 'quote_accepted' &&
    ...
    canBeModified(): boolean {
        return this.props.status.isPending() || this.props.status.getValue() === 'quote_sent';
    }
