...
export class PaymentMethodType {
  ...
  static create(value: string): PaymentMethodType {
    ...
    if (!this.validTypes.includes(normalizedValue)) {
      throw new Error(
        `Invalid payment method type: ${value}. Must be one of: ${this.validTypes.join(', ')}`,
      );
    }
  ...
  isCard(): boolean {
    return (
      this.value === PaymentMethodTypeEnum.CREDIT_CARD ||
  ...
  isDigitalWallet(): boolean {
    return (
      this.value === PaymentMethodTypeEnum.WALLET ||
...
export class CardDetails {
  ...
  private validate(): void {
    ...
    if (this.data.cardHolderName) {
      if (
        this.data.cardHolderName.length < 2 ||
        this.data.cardHolderName.length > 100
      ) {
        throw new Error(
          'Card holder name must be between 2 and 100 characters',
        );
      }
    }
  }
...
export class BankTransferDetails {
  ...
  private validate(): void {
    ...
    if (this.data.accountHolderName) {
      if (
        this.data.accountHolderName.length < 2 ||
        this.data.accountHolderName.length > 100
      ) {
        throw new Error(
