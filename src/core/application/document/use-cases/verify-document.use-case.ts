...
export class VerifyDocumentUseCase {
    ...
    async verify(dto: VerifyDocumentDTO): Promise<Document> {
        ...
        this.logger.log(
            `Document ${dto.documentId} verified by ${dto.verifiedBy}`
        );
    ...
    async verifyMultiple(dto: BulkVerifyDocumentsDTO): Promise<{ verified: number; failed: string[] }> {
        const failed: string[] = [];
        let verified = 0;
        ...
        for (const documentId of dto.documentIds) {
            try {
                ...
                this.logger.warn(`Failed to verify document ${documentId}: ${error.message}`);
