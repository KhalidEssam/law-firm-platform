...
import { CLOUDINARY_PROVIDER, S3_PROVIDER } from './storage-provider.interface';
...
export class DocumentStorageService {
  ...
  constructor(
    ...
    private readonly cloudinaryProvider?: IStorageProvider,
    @Optional()
    @Inject(S3_PROVIDER)
    private readonly s3Provider?: IStorageProvider,
  ) {
  ...
  private loadConfig(): StorageServiceConfig {
    ...
    const fallbackProviderStr = this.configService.get<string>(
  ...
  private registerProviders(): void {
    ...
    if (this.providers.size === 0) {
      this.logger.warn(
        'No storage providers registered. Document upload will fail.',
      );
    }
  }
