...
export class CloudinaryStorageService implements IStorageProvider {
  ...
  private loadConfig(): CloudinaryConfig | null {
    ...
    if (!cloudName || !apiKey || !apiSecret) {
      this.logger.warn(
        'Cloudinary credentials not configured. Document upload will be disabled.',
      );
      return null;
    }
  ...
  async delete(options: FileDeleteOptions): Promise<boolean> {
    ...
    const publicId =
      options.publicId || this.extractPublicIdFromUrl(options.fileUrl);
    ...
    try {
      ...
      if (success) {
        ...
        this.logger.warn(
          `Failed to delete file from Cloudinary: ${publicId}, result: ${result.result}`,
        );
      }
