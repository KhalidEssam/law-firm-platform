...
export class CreateLitigationCaseUseCase {
    ...
    async execute(command: CreateLitigationCaseCommand): Promise<any> {
        const litigationCase = LitigationCase.create({
            ...
            caseSubtype: command.caseSubtype ? CaseSubtype.create(command.caseSubtype) : undefined,
            title: CaseTitle.create(command.title),
            description: CaseDescription.create(command.description),
            courtName: command.courtName ? CourtName.create(command.courtName) : undefined,
            caseDetails: command.caseDetails ? CaseDetails.create(command.caseDetails) : undefined,
        });
...
export class UpdateLitigationCaseUseCase {
    ...
    async execute(command: UpdateLitigationCaseCommand): Promise<any> {
        ...
        if (command.description) {
            litigationCase.updateDescription(CaseDescription.create(command.description));
        }
    ...
    private toDto(litigationCase: LitigationCase): any {
        return new CreateLitigationCaseUseCase(this.repository as any)['toDto'](litigationCase);
    }
}
...
export class SendQuoteUseCase {
    ...
    async execute(command: SendQuoteCommand): Promise<any> {
        ...
        const quoteDetails = command.quoteDetails ? QuoteDetails.create(command.quoteDetails) : undefined;
...
export class GetLitigationCaseUseCase {
    ...
    async execute(query: GetLitigationCaseQuery): Promise<any> {
        const litigationCase = await this.repository.findById(CaseId.create(query.caseId));
        ...
        const isAssigned = litigationCase.assignedProviderId?.getValue() === query.userId;
