...
export class CreateLitigationWithNotificationUseCase {
    private readonly logger = new Logger(CreateLitigationWithNotificationUseCase.name);
...
export class AssignProviderWithNotificationUseCase {
    private readonly logger = new Logger(AssignProviderWithNotificationUseCase.name);
    ...
    async execute(command: AssignProviderCommand, caseDetails: {
        subscriberId: string;
        caseType: string;
        title: string;
    }): Promise<any> {
        ...
        if (this.notificationService) {
            try {
                ...
                await Promise.all([
                    this.notificationService.notifyLitigationCaseAssignedToSubscriber(payload),
                    this.notificationService.notifyLitigationCaseAssignedToProvider(payload),
                ]);
            } catch (error) {
...
export class SendQuoteWithNotificationUseCase {
    ...
    async execute(command: SendQuoteCommand, subscriberId: string, title: string, caseType: string): Promise<any> {
        // 1. Send the quote
...
export class ActivateCaseWithNotificationUseCase {
    private readonly logger = new Logger(ActivateCaseWithNotificationUseCase.name);
    ...
    async execute(
        ...
        if (this.notificationService) {
            try {
                ...
                this.logger.error('Failed to send case activation notification:', error);
            }
        }
    ...
    async execute(
        ...
        if (this.notificationService) {
            try {
                ...
                this.logger.error('Failed to send case cancellation notification:', error);
            }
        }
...
export class ProcessRefundWithNotificationUseCase {
    private readonly logger = new Logger(ProcessRefundWithNotificationUseCase.name);
