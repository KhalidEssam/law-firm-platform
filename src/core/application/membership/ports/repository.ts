...
export interface IMembershipRepository {
    ...
    toggleAutoRenew(membershipId: string, autoRenew: boolean): Promise<Membership>;
...
export interface IMembershipPaymentRepository {
    ...
    findByMembership(membershipId: string, options?: {
        status?: string;
        limit?: number;
        offset?: number;
    ...
    markAsCompleted(paymentId: string, providerTxnId: string): Promise<MembershipPayment>;
    markAsFailed(paymentId: string, reason?: string): Promise<MembershipPayment>;
    markAsRefunded(paymentId: string, refundReason?: string): Promise<MembershipPayment>;
...
export interface IMembershipCouponRepository {
    ...
    // Business Logic
    validateCoupon(code: string): Promise<{ valid: boolean; reason?: string; coupon?: MembershipCoupon }>;
}
...
export interface IMembershipCouponRedemptionRepository {
    // Create
    create(redemption: MembershipCouponRedemption): Promise<MembershipCouponRedemption>;
    ...
    hasUserRedeemedCoupon(membershipId: string, couponId: string): Promise<boolean>;
...
export interface IMembershipQuotaUsageRepository {
    ...
    findCurrentByMembership(membershipId: string): Promise<MembershipQuotaUsage | null>;
    findByMembership(membershipId: string, options?: {
        startDate?: Date;
        endDate?: Date;
    ...
    incrementUsage(membershipId: string, resource: QuotaResource, amount: number): Promise<MembershipQuotaUsage>;
