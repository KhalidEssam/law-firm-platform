...
export class MembershipIntegrationService {
    ...
    async validateMembershipForService(
        ...
        if (!membership) {
            return {
                ...
                quotaUsed: 0,
                isUnlimited: false,
                errorMessage: 'No active membership found. Please subscribe to a membership plan.',
            };
        }
        ...
        if (!membership.isActive) {
            return {
                ...
                quotaUsed: 0,
                isUnlimited: false,
                errorMessage: 'Your membership is not active. Please renew your subscription.',
            };
        }
        ...
        if (membership.endDate && membership.endDate < new Date()) {
            return {
                ...
                quotaUsed: 0,
                isUnlimited: false,
                errorMessage: 'Your membership has expired. Please renew your subscription.',
            };
        }
    ...
    async recordServiceUsage(dto: RecordUsageDto): Promise<UsageRecordResult> {
        ...
        const requestTypeMap: Record<ServiceType, 'consultation' | 'legal_opinion' | 'service' | 'litigation' | 'call'> = {
        ...
        if (alreadyRecorded) {
            throw new BadRequestException('Service usage already recorded for this request');
        }

        // 4. Calculate period dates
        const now = new Date();
        const periodStart = new Date(now.getFullYear(), now.getMonth(), 1);
        const periodEnd = membership.endDate || new Date(now.getFullYear(), now.getMonth() + 1, 0);
    ...
    // ============================================
    async canUseService(userId: string, serviceType: ServiceType): Promise<boolean> {
        const validation = await this.validateMembershipForService(userId, serviceType);
