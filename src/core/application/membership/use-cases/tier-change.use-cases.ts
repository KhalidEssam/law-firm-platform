  ...
  async execute(dto: ChangeTierDto): Promise<TierChangeResult> {
    ...
    if (!membership.isActive) {
      throw new BadRequestException(
        'Cannot change tier on inactive membership',
      );
    }
    ...
    if (!newTier.canBeSubscribed()) {
      throw new BadRequestException(
        'New tier is not available for subscription',
      );
    }

    // Determine if upgrade or downgrade
    const isUpgrade = newTier.price.amount > (oldTier?.price.amount || 0);
    const changeReason = isUpgrade
      ? MembershipChangeReason.UPGRADE
      : MembershipChangeReason.DOWNGRADE;
...
export class UpgradeMembershipUseCase {
  ...
  async execute(
    ...
    if (currentTier && newTier.price.amount <= currentTier.price.amount) {
      throw new BadRequestException(
        'New tier must have higher price for upgrade. Use downgrade instead.',
      );
    }
...
export class DowngradeMembershipUseCase {
  ...
  async execute(
    ...
    if (currentTier && newTier.price.amount >= currentTier.price.amount) {
      throw new BadRequestException(
        'New tier must have lower price for downgrade. Use upgrade instead.',
      );
    }
...
export class GetTierChangeStatisticsUseCase {
  ...
  async execute(
    ...
    const stats = await this.changeLogRepo.getTierChangeStats(
      startDate,
      endDate,
    );
    return {
