...
export class ResumeMembershipUseCase {
    ...
    async execute(dto: ResumeMembershipDto): Promise<Membership> {
        ...
        const lastChange = await this.changeLogRepo.getLatestChange(dto.membershipId);
        let extensionDays = 0;

        if (dto.extendEndDate && lastChange?.reason === MembershipChangeReason.PAUSE) {
            const pausedAt = lastChange.changedAt;
            extensionDays = Math.ceil((Date.now() - pausedAt.getTime()) / (1000 * 60 * 60 * 24));
        }
        ...
        if (renewDuration > 0) {
            ...
            return await this.membershipRepo.toggleAutoRenew(dto.membershipId, membership.autoRenew);
        }
    }
}
...
export class ReactivateMembershipUseCase {
    ...
    async execute(membershipId: string, durationInMonths: number, reactivatedBy?: string): Promise<Membership> {
        ...
        if (membership.isActive) {
            throw new BadRequestException('Membership is already active. Use renew instead.');
        }
