...
export class GetMembershipStatisticsUseCase {
  ...
  async execute(): Promise<MembershipStatisticsDto> {
    ...
    for (const tier of tiers) {
      ...
      const activeForTier = await this.membershipRepo.count({
        tierId: tier.id,
        isActive: true,
      });
      byTier.push({
    ...
    // Get change stats
    const changeStats = await this.changeLogRepo.getTierChangeStats(
      startOfMonth,
      endOfMonth,
    );
...
export class GetTierDistributionUseCase {
  ...
  > {
    ...
    for (const tier of tiers) {
      ...
      const percentage =
        totalActive > 0 ? (activeCount / totalActive) * 100 : 0;
