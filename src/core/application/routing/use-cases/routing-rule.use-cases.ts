...
import {
  RequestType,
  isValidRequestType,
} from '../../../domain/routing/value-objects/request-type.vo';
import {
  RoutingStrategy,
  isValidRoutingStrategy,
} from '../../../domain/routing/value-objects/routing-strategy.vo';
import {
  ...
  async execute(dto: CreateRoutingRuleDto): Promise<RoutingRule> {
    ...
    if (await this.ruleRepo.existsByName(dto.name)) {
      throw new ConflictException(
        `A routing rule with name "${dto.name}" already exists`,
      );
    }
...
export class FindApplicableRuleUseCase {
  ...
  async execute(context: RequestContext): Promise<RoutingRule | null> {
    ...
    const rules = await this.ruleRepo.findActiveByRequestType(
      context.requestType,
    );
