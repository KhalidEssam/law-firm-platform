...
export class AutoAssignRequestUseCase {
  ...
  async execute(dto: AutoAssignRequestDto): Promise<AssignmentResult> {
    ...
    const allProviders = await this.dataProvider.getProviderInfoForRouting(
      dto.requestType,
    );
    ...
    const selectedProvider = matchingRule.selectProvider(
      eligibleProviders,
      lastAssignedIndex,
    );
    ...
    if (matchingRule.routingStrategy === RoutingStrategy.ROUND_ROBIN) {
      const providerIndex = eligibleProviders.findIndex(
    ...
    const providerUser = await this.dataProvider.getProviderUserDetails(
      selectedProvider.id,
    );
    ...
    return {
      ...
      providerId: selectedProvider.id,
      providerName:
        providerUser?.fullName || providerUser?.username || 'Unknown',
      ruleId: matchingRule.id,
      ruleName: matchingRule.name,
      strategy: matchingRule.routingStrategy,
    };
...
export class ReassignRequestUseCase {
  ...
  async execute(dto: ReassignRequestDto): Promise<AssignmentResult> {
    ...
    const providerCheck = await this.dataProvider.verifyProviderAvailable(
      dto.newProviderId,
    );
    ...
    return {
      ...
      providerId: dto.newProviderId,
      providerName:
        providerCheck.fullName || providerCheck.username || 'Unknown',
...
export class GetProviderWorkloadUseCase {
  ...
  async execute(): Promise<ProviderWorkload[]> {
    ...
    for (const p of providers) {
      const [active, pending, inProgress, completedToday, rating] =
        await Promise.all([
          this.dataProvider.getProviderActiveRequestCount(p.userId),
          this.dataProvider.getProviderRequestCountByStatus(p.userId, [
            'pending',
          ]),
          this.dataProvider.getProviderRequestCountByStatus(p.userId, [
            'in_progress',
...
export class GetRoutingStatsUseCase {
  ...
  async execute(): Promise<{
    ...
    for (const rule of rules.data) {
      ...
      rulesByRequestType[data.requestType] =
        (rulesByRequestType[data.requestType] || 0) + 1;

      // Count by strategy
      rulesByStrategy[data.routingStrategy] =
        (rulesByStrategy[data.routingStrategy] || 0) + 1;
