...
} from '../../../domain/notification/ports/message-template.repository';
  ...
  async activeByCode(code: string): Promise<MessageTemplate> {
    ...
    if (!template) {
      throw new NotFoundException(
        `Active template with code '${code}' not found`,
      );
    }
    return template;
  }
...
export class RenderMessageTemplateUseCase {
  ...
  async execute(dto: RenderTemplateDto): Promise<{
    ...
    if (!template) {
      throw new NotFoundException(
        `Active template with code '${dto.templateCode}' not found`,
      );
    }
