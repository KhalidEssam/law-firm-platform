...
import {
  ...
  CreateCallRequestWithMembershipResult,
} from '../membership';
...
export interface CreateCallRequestWithRoutingResult
  extends CreateCallRequestWithMembershipResult {
...
export class CreateCallRequestWithRoutingUseCase {
  private readonly logger = new Logger(
    CreateCallRequestWithRoutingUseCase.name,
  );
  ...
  async execute(
  ...
  ): Promise<CreateCallRequestWithRoutingResult> {
    ...
    if (this.routingService) {
      try {
        ...
        this.logger.error(
          `Auto-assignment failed for call request ${result.callRequest.id}:`,
          error,
        );
      }
    }
