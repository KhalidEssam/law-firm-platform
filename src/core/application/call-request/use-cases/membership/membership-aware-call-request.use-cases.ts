...
export class CreateCallRequestWithMembershipUseCase {
    private readonly logger = new Logger(CreateCallRequestWithMembershipUseCase.name);
    ...
    async execute(dto: CreateCallRequestDto): Promise<CreateCallRequestWithMembershipResult> {
        ...
        if (this.membershipService) {
            ...
            if (!validation.isValid) {
                throw new BadRequestException(
                    validation.errorMessage || 'Cannot create call request: membership validation failed',
                );
            }
        }
        ...
        if (this.membershipService && validation?.membershipId) {
            try {
                ...
                this.logger.log(`Call request ${callRequest.id} created for user ${dto.subscriberId}`);
            } catch (error) {
...
export class EndCallWithUsageTrackingUseCase {
    ...
    async execute(
        ...
        const minutesUsed = actualDurationMinutes ?? this.calculateDuration(callRequest);
