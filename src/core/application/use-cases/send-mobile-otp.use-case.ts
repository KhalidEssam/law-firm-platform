...
export class SendMobileOtpUseCase {
    ...
    async execute(dto: SendMobileOtpDto): Promise<SendMobileOtpResult> {
        ...
        if (!this.isValidPhoneNumber(phoneNumber)) {
            throw new BadRequestException('Invalid phone number format. Use international format (e.g., +966501234567)');
        }

        // 3. Check for existing OTP (cooldown period)
        const existingOtp = await this.otpRepository.getOtp(dto.userId, phoneNumber);
        if (existingOtp && existingOtp.expiresAt > new Date()) {
            ...
            const minSecondsBeforeResend = (OTP_CONFIG.EXPIRY_MINUTES * 60) - OTP_CONFIG.RESEND_COOLDOWN_SECONDS;
        ...
        // 5. Store OTP
        await this.otpRepository.storeOtp(dto.userId, phoneNumber, otpCode, expiresAt);
