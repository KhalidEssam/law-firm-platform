...
import { AggregateMetricDto, AggregatedMetricDto } from '../../dto/reports.dto';
...
export class AggregateMetricsUseCase {
  ...
  async execute(
    dto: AggregateMetricDto,
  ): Promise<AggregatedMetricDto | AggregatedMetricDto[]> {
    ...
    // Simple aggregation
    const result = await this.metricRepo.aggregate(dto.metricName, timeRange, {
      groupBy: dto.groupBy,
    });
