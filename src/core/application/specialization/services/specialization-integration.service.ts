...
export class SpecializationIntegrationService {
    ...
    async getProviderSpecializationNames(providerId: string): Promise<string[]> {
        ...
        try {
            const providerSpecs = await this.providerSpecializationRepo.findByProvider(providerId);
            ...
            this.logger.error(`Failed to get specializations for provider ${providerId}:`, error);
            return [];
        }
    }
     ...
     */
    async getProviderExpertiseInfo(providerId: string): Promise<ProviderWithSpecializations | null> {
        if (!this.providerSpecializationRepo || !this.specializationRepo) {
            return null;
        }

        try {
            const expertise = await this.providerSpecializationRepo.getProviderExpertise(providerId);
            ...
            for (const ps of expertise.specializations) {
                const spec = await this.specializationRepo.findById(ps.specializationId);
            ...
            this.logger.error(`Failed to get expertise for provider ${providerId}:`, error);
            return null;
        }
    }
    ...
    async findProvidersWithSpecializations(
        ...
        try {
            ...
            for (const specId of specializationIds) {
                const providerSpecs = await this.providerSpecializationRepo.findBySpecialization({
                ...
                for (const ps of providerSpecs) {
                    ...
                    if (matches.has(ps.providerId)) {
                        ...
                        existing.matchScore += 1 + (ps.isCertified ? 0.5 : 0) + (ps.successRate || 0) / 100;
                        if (ps.isCertified) {
                            existing.isCertified = true;
                        }
                        if (ps.experienceYears && ps.experienceYears > existing.experienceYears) {
                        ...
                        matches.set(ps.providerId, {
                            ...
                            matchScore: 1 + (ps.isCertified ? 0.5 : 0) + (ps.successRate || 0) / 100,
            ...
            this.logger.error('Failed to find providers with specializations:', error);
            return [];
        }
    }
    ...
    async getTopProvidersByCategory(
        ...
        try {
            const specializations = await this.specializationRepo.findByCategory(category);
            ...
            return this.findProvidersWithSpecializations(specializationNames, { limit });
        } catch (error) {
            this.logger.error(`Failed to get top providers for category ${category}:`, error);
            return [];
        }
    }
    ...
    async providerHasSpecializations(
        ...
        try {
            const providerSpecs = await this.getProviderSpecializationNames(providerId);

            if (requiredSpecializations.length === 0) {
                return true;
            }

            const normalizedRequired = requiredSpecializations.map(s => s.toLowerCase());
            ...
            this.logger.error(`Failed to check specializations for provider ${providerId}:`, error);
    ...
    async recordCaseResult(
        ...
        try {
            ...
            for (const spec of specializations) {
                const providerSpec = await this.providerSpecializationRepo.findByProviderAndSpecialization(
                    providerId,
                    spec.id
                );
                ...
                if (providerSpec) {
                    ...
                    const updated = await this.providerSpecializationRepo.incrementCaseCount(providerSpec.id);

                    const currentRate = providerSpec.successRate ?? 0;
                    const oldCount = providerSpec.caseCount;
                    const newCount = updated.caseCount;

                    const successfulCases = (currentRate / 100) * oldCount + (wasSuccessful ? 1 : 0);
