...
export class CreateProviderSpecializationUseCase {
    ...
    async execute(dto: CreateProviderSpecializationDTO): Promise<ProviderSpecialization> {
        // Verify specialization exists
        const specialization = await this.specializationRepository.findById(dto.specializationId);
        if (!specialization) {
            throw new NotFoundException(`Specialization with ID "${dto.specializationId}" not found`);
        }
