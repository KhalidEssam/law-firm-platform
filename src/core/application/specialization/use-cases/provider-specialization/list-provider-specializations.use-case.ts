...
export class ListProviderSpecializationsUseCase {
    ...
    async execute(dto?: ListProviderSpecializationsDTO): Promise<ListProviderSpecializationsResult> {
    ...
    async executeByProvider(providerId: string): Promise<ProviderSpecialization[]> {
        return await this.repository.findByProvider(providerId);
    }

    async executeCertifiedByProvider(providerId: string): Promise<ProviderSpecialization[]> {
        return await this.repository.findCertifiedByProvider(providerId);
    }

    async getProviderExpertise(providerId: string): Promise<ProviderExpertiseResult> {
    ...
    async findTopProvidersBySpecialization(
        ...
        return await this.repository.findTopProvidersBySpecialization(specializationId, limit);
    }

    async countProvidersBySpecialization(specializationId: string): Promise<number> {
        return await this.repository.countProvidersBySpecialization(specializationId);
    }
}
