...
export class RecordCaseResultUseCase {
    ...
    async execute(dto: RecordCaseResultDTO): Promise<ProviderSpecialization> {
        ...
        const updatedSpecialization = await this.repository.incrementCaseCount(providerSpecialization.id);
        ...
        const successfulCases = (currentRate / 100) * oldCaseCount + (dto.wasSuccessful ? 1 : 0);
