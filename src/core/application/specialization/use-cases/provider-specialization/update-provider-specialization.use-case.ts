...
export class UpdateProviderSpecializationUseCase {
    ...
    async execute(dto: UpdateProviderSpecializationDTO): Promise<ProviderSpecialization> {
        const providerSpecialization = await this.repository.findById(dto.id);

        if (!providerSpecialization) {
            throw new NotFoundException(`Provider specialization with ID "${dto.id}" not found`);
        }
    ...
    async addCertification(id: string, certification: CertificationDetail): Promise<ProviderSpecialization> {
    ...
    async removeCertification(id: string, certificationName: string): Promise<ProviderSpecialization> {
        const providerSpecialization = await this.repository.findById(id);
