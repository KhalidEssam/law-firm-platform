...
export class CreateSLAPolicyUseCase {
  ...
  async execute(dto: CreateSLAPolicyDto): Promise<SLAPolicyResponseDto> {
    ...
    if (
      await this.policyRepository.existsByTypeAndPriority(requestType, priority)
    ) {
      throw new ConflictException(
...
export class GetSLAPolicyByTypeUseCase {
  ...
  ): Promise<SLAPolicyResponseDto | null> {
    const type =
    ...
    if (dto?.requestType) {
      ...
      if (!type) {
        throw new BadRequestException(
          `Invalid request type: ${dto.requestType}`,
        );
      }
  ...
  async execute(): Promise<{ created: number; skipped: number }> {
    ...
    for (const requestType of requestTypes) {
      for (const priority of priorities) {
        const exists = await this.policyRepository.existsByTypeAndPriority(
          requestType,
          priority,
        );
        if (exists) {
