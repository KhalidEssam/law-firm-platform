...
export class SLAIntegrationService {
    ...
    checkSLAStatus(data: RequestSLAData): SLAStatusCheckResult {
        ...
        if (data.resolvedAt) {
            const wasBreached = data.slaDeadline && data.resolvedAt > data.slaDeadline;
            return {
        ...
        return {
            currentStatus: SLAStatus.ON_TRACK,
            previousStatus,
            hasChanged: previousStatus !== SLAStatus.ON_TRACK && previousStatus !== null,
            isBreached: false,
            isAtRisk: false,
            timeToDeadline,
        };
    ...
    getUrgencyScore(data: RequestSLAData): number {
        ...
        const totalTime = new Date(data.slaDeadline!).getTime() - new Date(data.createdAt).getTime();
