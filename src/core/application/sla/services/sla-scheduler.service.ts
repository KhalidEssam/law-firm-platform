...
export class SLASchedulerService implements OnModuleInit {
  ...
  async handleSLAStatusCheck(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `SLA scheduled check failed: ${error.message}`,
        error.stack,
      );
    } finally {
  ...
  async handleDailySLAReport(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Daily SLA report failed: ${error.message}`,
        error.stack,
      );
    }
  }
    ...
    const requestTypes = [
      {
        ...
        checker: () => this.checkConsultationSLAs(),
      },
      {
        name: 'LegalOpinionRequest',
        checker: () => this.checkLegalOpinionSLAs(),
      },
  ...
  private async triggerBreachNotification(
    ...
    if (!this.notificationService) {
      this.logger.warn(
        'Notification service not available for SLA breach notification',
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `SLA breach notification sent for ${requestType} ${request.requestNumber}`,
      );
    } catch (error) {
      this.logger.error(
  ...
  private async triggerAtRiskWarning(
    ...
    if (!this.notificationService) {
      this.logger.warn(
        'Notification service not available for SLA at-risk warning',
      );
      return;
    }
    ...
    try {
      const hoursRemaining = request.slaDeadline
        ? Math.max(
            0,
            (request.slaDeadline.getTime() - Date.now()) / (1000 * 60 * 60),
          )
        : 0;
      ...
      this.logger.log(
        `SLA at-risk warning sent for ${requestType} ${request.requestNumber}`,
      );
    } catch (error) {
