...
import { MembershipIntegrationService, ServiceType } from '../../membership/services/membership-integration.service';
...
export class GenerateServiceUsageInvoiceUseCase {
    ...
    async execute(dto: GenerateUsageInvoiceDto): Promise<MembershipInvoice | null> {
        // 1. Get unbilled usage
        const unbilledUsage = await this.membershipService.getUnbilledUsage(dto.membershipId);
        ...
        if (dto.includeUsageIds && dto.includeUsageIds.length > 0) {
            usageToInvoice = unbilledUsage.filter(u => dto.includeUsageIds!.includes(u.id));
        }
...
export class GetBillableUsageByServiceTypeUseCase {
    ...
    async execute(userId: string, serviceType?: ServiceType): Promise<{
        consultation: number;
        legalOpinion: number;
        litigation: number;
        call: number;
...
export class GetCombinedBillingSummaryUseCase {
    ...
    async execute(userId: string): Promise<{
        ...
        const unpaidInvoices = await this.invoiceRepo.findUnpaidByMembershipId(status.membershipId);
        ...
        const invoiceTotal = result.pendingInvoices.reduce((sum, inv) => sum + inv.amount, 0);
