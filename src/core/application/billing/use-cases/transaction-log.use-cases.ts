...
export class MarkTransactionFailedUseCase {
    ...
    async execute(id: string, errorDetails?: Record<string, unknown>): Promise<TransactionLog> {
        const transaction = await this.transactionRepository.findById(id);
        if (!transaction) {
            throw new NotFoundException(`Transaction with ID ${id} not found`);
        }
        ...
        if (errorDetails) {
            updatedTransaction = updatedTransaction.withMetadata({ error: errorDetails });
        }
...
export class GetTotalSpentUseCase {
    ...
    async execute(userId: string, fromDate?: Date, toDate?: Date): Promise<number> {
        return await this.transactionRepository.getTotalSpent(userId, fromDate, toDate);
    }
}
