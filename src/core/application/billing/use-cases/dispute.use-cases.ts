...
export class CreateDisputeUseCase {
  ...
  async execute(dto: CreateDisputeDto): Promise<Dispute> {
    ...
    const relatedEntityType = dto.consultationId
      ? 'consultation'
      : dto.legalOpinionId
        ? 'legal_opinion'
        : dto.serviceRequestId
    ...
    if (hasActiveDispute) {
      throw new BadRequestException(
        'An active dispute already exists for this entity',
      );
    }
...
export class UpdateDisputePriorityUseCase {
  ...
  async execute(_id: string, dto: UpdateDisputePriorityDto): Promise<Dispute> {
    ...
    if (dispute.status.isFinal()) {
      throw new BadRequestException(
        'Cannot update priority of a finalized dispute',
      );
    }
...
export class AddDisputeEvidenceUseCase {
  ...
  async execute(_id: string, dto: AddDisputeEvidenceDto): Promise<Dispute> {
    ...
    if (dispute.status.isFinal()) {
      throw new BadRequestException(
        'Cannot add evidence to a finalized dispute',
      );
    }
