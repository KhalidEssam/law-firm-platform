    ...
    async execute(command: CreateOpinionRequestCommand): Promise<any & {
        ...
        if (!validation.isValid) {
            throw new BadRequestException(
                validation.errorMessage || 'Cannot create legal opinion request: membership validation failed',
            );
        }
    ...
    async execute(opinionId: string, clientId: string, chargedAmount?: number): Promise<void> {
        ...
        if (chargedAmount && chargedAmount > 0) {
            try {
                ...
                const status = await this.membershipService.getMembershipStatus(clientId);
