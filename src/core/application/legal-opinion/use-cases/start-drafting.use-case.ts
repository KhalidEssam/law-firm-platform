...
// ============================================
// START DRAFTING USE CASE
// ============================================

export interface StartDraftingCommand {
  opinionRequestId: string;
  lawyerId: string;
}

@Injectable()
export class StartDraftingUseCase {
  constructor(
    @Inject('ILegalOpinionRequestRepository')
    private readonly repository: ILegalOpinionRequestRepository,
  ) {}

  async execute(command: StartDraftingCommand): Promise<any> {
