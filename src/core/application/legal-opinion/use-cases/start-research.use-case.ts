...
export class StartResearchUseCase {
  ...
  async execute(command: StartResearchCommand): Promise<any> {
  ...
  private toDto(opinion: LegalOpinionRequest): any {
    return {
      ...
      researchStartedAt: opinion.researchStartedAt?.toISOString(),
